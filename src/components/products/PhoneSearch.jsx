...
const PhoneSearch = () => {
  ...
  return (
    <form className="flex flex-row justify-between w-full order-1 mt-2 relative">
      <input
        type="text"
        placeholder="search products here..."
        className="flex-1 w-full border-[#bebebe] border-2 focus:border-[#408A9E] outline-none rounded p-2"
        name={search}
        onChange={(e) => setSearch(e.target.value)}
      />
      <button className="main_btn absolute right-0 p-3" onClick={handle}>search</button>
